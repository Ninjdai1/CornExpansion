...
public class FoodUtils {
    ...
    public static FoodProperties createFCFood(int nutrition, float saturationMod, ResourceLocation effectId, int duration, boolean alwaysEat, boolean fast) {
        FoodProperties.Builder food = new FoodProperties.Builder().nutrition(nutrition).saturationModifier(saturationMod);
        if (alwaysEat) food.alwaysEdible();
        if (fast) food.fast();
        food.effect(MobEffectRegistry.inst(effectId, duration), 1.0F);
        return food.build();
    }
